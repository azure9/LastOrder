...
namespace ListCtrlEx
{
	...
	public:
		...
		public:
			...
			CellData(BOOL bEnabled=TRUE, BOOL bChecked=FALSE, int nRadioGroup=0, int nImage=-1, UINT uProgMaxVal=100, COLORREF clrREF = 0) 
				: m_nRadioGroup(nRadioGroup), m_strValidChars(_T("")),
				m_nImage(nImage), m_uProgresVal(0), m_uProgressMax(uProgMaxVal),clrREF(0)
			{
				SetEnable(bEnabled);
				SetCheck(bChecked);
			};
			...
			inline void SetColorRef(COLORREF clr=0){
				clrREF = clr;
			}
...
// operations
	public:
		...
		//for line color
		void				SetLineColor(int nRow, COLORREF clrREF);
		// for normal & edit box
