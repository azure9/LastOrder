...
void CListCtrlEx::SetLineColor(int nRow, COLORREF clrREF)
{
	//FindCellData(nRow, nCol).m_bEnabled=bEnabled;
	int cnt = GetColumnCount();
	for (int nCol=0;nCol<cnt;++nCol)
	{
		FindCellData(nRow, nCol).SetColorRef(clrREF);
...
void CListCtrlEx::DrawNormal(CDC *pDC, CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData, int uLvcFmt)
{
	...
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):cellData.GetColorRef();
	...
	if(uLvcFmt & LVCFMT_RIGHT)
	{
		nFormat=DT_RIGHT;
	}
	else if(uLvcFmt & LVCFMT_CENTER)
	{
		nFormat=DT_CENTER;
	}
...
void CListCtrlEx::DrawCheckBox(CDC *pDC,  CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData)
{
	...
	COLORREF crCheckMark=cellData.GetEnable()?RGB(0x66,0xcc,0xff):RGB(192,192,192);//选中的项目
